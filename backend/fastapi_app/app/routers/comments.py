...
async def get_comments(
    ...
    for comment in comments:
        # Count upvotes
        comment.upvotes = db.query(func.count(models.Upvote.id)).filter(
            models.Upvote.comment_id == comment.id
        ).scalar()
        
        # Check if current user has upvoted
        if current_user:
            comment.has_upvoted = db.query(models.Upvote).filter(
                models.Upvote.comment_id == comment.id,
        ...
        comment.mentioned_users = [user.username for user in comment.mentions]
        
        # Process replies the same way
        for reply in comment.replies:
            reply.upvotes = db.query(func.count(models.Upvote.id)).filter(
                models.Upvote.comment_id == reply.id
            ).scalar()
            
            if current_user:
                reply.has_upvoted = db.query(models.Upvote).filter(
                    models.Upvote.comment_id == reply.id,
                    models.Upvote.user_id == current_user.id
                ).first() is not None
            else:
                reply.has_upvoted = False
                
            reply.mentioned_users = [user.username for user in reply.mentions]
...
async def create_comment(
    ...
    # Add computed fields for the response
    db_comment.upvotes = 0
    db_comment.has_upvoted = False
    db_comment.replies = []
    db_comment.mentioned_users = [user.username for user in db_comment.mentions]
...
async def update_comment(
    ...
    # Add replies with computed fields
    comment.replies = db.query(models.Comment).filter(
